...
char old_prefix[BGPDUMP_ADDRSTRLEN];
int old_length=-1;
...
int main(int argc, char **argv) {  
	...
	my_dump=bgpdump_open_dump(argv[1]);
			...
			fprintf( stderr, "Usage: ./parseBGP <dump_file>\n" );
			exit( 1 );
    }
...
//    		printf("    PREFIX      : %s/%d\n",prefix,e->prefix_length);
    		if( strcmp(prefix,old_prefix)==0 && old_length==e->prefix_length ) continue;
    		printf(" %s/%d\n",prefix,e->prefix_length );
